...
{
    static bool verbose;
    static string baseDirPath = null!;
    ...
    static void Main(string[] args)
    {
        ...
        options.AddOptional("confirmation", "", new[] { "-c", "--confirm" }, help:"Show a confirmation question before run command.");
	    ...
	    if (args.Length > 0)
	    {
            ...
            var confirmation = options.Get<string>("confirmation");
			...
			Console.WriteLine("Path to process: " + dirOrFile);
			if (!string.IsNullOrWhiteSpace(confirmation))
            {
				...
				while (true)
                {
					...
					if (key.Key == ConsoleKey.N) Environment.Exit(1);
                }
            }
    ...
    static void help(CommandLineOptions options)
    {
	    ...
	    Console.WriteLine("");
	    Console.WriteLine("        multirun <dirOrFile> [ <arguments> ] <command> -- <commandArgs>");
	    Console.WriteLine("");
    ...
    static string encodeParameter(string argument)
    {
